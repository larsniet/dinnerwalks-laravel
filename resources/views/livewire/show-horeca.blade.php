                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                ...
                                <button
                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                    aria-label="Delete" wire:click="$emit('remove',{{ $horeca }})">
                                    <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
...
@push('scripts')
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            @this.on('remove', horeca => {
                Swal.fire({
                    title: 'Weet je het zeker?',
                    text: horeca.naam + ' gaat je wel ziek erg missen :(',
                    icon: "warning",
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#aaa',
                    confirmButtonText: 'Ja, ' + horeca.naam + ' was toch niet leuk.',
                    cancelButtonText: "Uh, nee?"
                }).then((result) => {
                    //if user clicks on delete
                    if (result.value) {
                        // calling destroy method to delete
                        @this.call('remove', horeca)
                        // success response
                        Swal.fire({
                            title: horeca.naam + ' succesvol verwijderd!',
                            icon: 'success'
                        });
                    } else {
                        return;
